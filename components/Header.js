	...
	return (
		<>
			<header className='w-full px-6 border-b-2 border-black flex justify-between justify-between items-center text-sm font-messapia font-bold'>
				...
				<button
					onClick={toggleNav}
					className='md:hidden icone-toggle bg-transparent rounded-full h-12 w-12'
				>
					<img
						src='/img/menu.png'
						alt='menu hamburger'
				...
				<div className='w-1/3 mx-auto flex justify-center p-3'>
					<Link href={'/'}>
						<Image height={70} width={150} src={logoHeader}
							alt='Belong - Paris' />
					</Link>
				</div>
				<div className='w-1/3  flex items-center justify-end space-x-5 ml-auto'>
					<Link href={'https://www.instagram.com/belongparis/'}>
						<a>
							<svg
								className='hidden md:block h-[30px] w-[30px]'
								xmlns='http://www.w3.org/2000/svg'
					...
					<Link href={'/contact'}>
						<a className='font-bold text-xs lg:text-[14px] animate-changeColor ml-auto'>
							Say Hi !
						</a>
					</Link>
				</div>
