...
function Footer() {
	...
	return (
		<footer className='bg-belrose text-belorange mt-auto flex justify-between items-center py-3 px-8 pb-10'>
			<div className='flex flex-col items-start'>
				<Link href={'/contact'}>Contact</Link>
				<div className="flex w-full justify-between">
					<Link className='w-1/2' href={`${pathname}`} locale={'fr'}>FR</Link>
					<Link className='w-1/2' href={`${pathname}`} locale={'en'}>EN</Link>
				</div>
			</div>
			<svg
			...
			<div className='flex flex-col items-end'>
				<Link href={'https://www.instagram.com/belongparis/'}>Instagram</Link>
				<Link href={'https://www.linkedin.com/company/belongparis/'}>Linkedin</Link>
			</div>
		</footer>
	);
}
